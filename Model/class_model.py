...
class BC_agent:
    def __init__(self,o_dim,a_dim,args):
        ...
        self.v = Vnet(self.o_dim,  self.hidden_size, self.n_support).to(args.device_train)
    ...
    def train_QBC(self,batch,cql=False):
        ...
        with torch.no_grad():
            ...
            weight = (torch.exp(torch.min((q_values_A - v_value), (q_values_B - v_value)) / 1.0) - torch.ones_like(q_values_A)).clamp(0.0, 0.5)
    ...
    def q_train(self,state_batch,action_batch,reward_batch,next_state_batch,done_batch):
        ...
        critic1_loss = cal_critic_loss(self.q1(state_batch, action_batch), target_q, use_quantile_critic=True)
        critic2_loss = cal_critic_loss(self.q2(state_batch, action_batch), target_q, use_quantile_critic=True)
    ...
    def v_train(self,state_batch,action_batch,reward_batch,next_state_batch,done_batch):
        ...
        with torch.no_grad():
            ...
            _, idx = torch.min(input=torch.cat([q_values_A, q_values_B], dim=1), dim=1, keepdim=True)
            mixed_dist = torch.stack([q_values_dist_A, q_values_dist_B], dim=1)
            idx = idx.repeat(1, self.n_support)

            idx = idx.view(self.batch_size, 1, self.n_support)
            next_dist = torch.gather(mixed_dist, dim=1, index=idx).view(q_values_dist_A.shape[0],q_values_dist_A.shape[1])

        v_loss = cal_critic_loss(v_value_dist, next_dist, use_quantile_critic=True)
